...
![](./images/a1fig1_labexperiment.png){width=250px}

Then you had to fit a straight line through the set of observations using a visual "best fit".

![](./images/a1fig2_labexperiment_withline.png){width=250px}
...
![](./images/a1fig3_labexperiment_slopeintercept.png){width=250px}
...
![](./images/a1fig4_twolabexperiments.png){width=250px}
...
df_loansf_1 = data.frame(df_loansf_1)
```

```{r}
df_loansf_1[1:10,]
ggplot(df_loansf_1, aes(FICO.Score, Interest.Rate), size=1) + geom_point(size=1)
```
...
But for now let's look at Y as a function of two independent variables, $X_{1}$ and $X_{2}$, so

Y=$a_{0} + a_{1}X_{1} + a_{2}X_{2}$

Here $a_{0}$ is the Intercept term and 
$a_{1}$,$a_{2}$ are the coefficients of $X_{1}$,$X_{2}$, the independent variables respectively.
